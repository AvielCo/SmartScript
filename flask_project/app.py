...
# MongoDB fields
cluster = MongoClient(
    "mongodb+srv://" + MONGO_USERNAME + ":" + MONGO_PASS + "@" + DOMAIN + "-gfohi.mongodb.net/test?retryWrites=true&w=majority")
...
class RegisterForm(Form):
    ...
    username = StringField('Username', [
        ...
        validators.Regexp(regex=USERNAME_REGEX,
                          message="Username must contain only letters and digits, starts with a letter.")])
    ...
    password = PasswordField('Password', [
        ...
        validators.Regexp(regex=PASSWORD_REGEX,
                          message="Password must contain at least one digit, one uppercase and one lowercase letters.")
    ])
...
def register():
    ...
    if request.method == 'POST' and form.validate():  # Method == POST means that the action has been redirected from the form and not from URL (GET method)
        ...
        collection.insert_one({"_id": _id, "name": name, "email": email, "username": username, "password": password,
                               "registeration_time": reg_time, "confirmed": confirmed, "admin": admin})
        flash("You are now registered, a verification mail sent to {}. Please check your inbox".format(email),
              "success")
        return redirect(url_for('login'))
    return render_template('register.html',
                           form=form)  # In case the form is not valid, or the request method is not POST
    ...
    if request.method == 'POST' and form.validate():
        ...
        if result != None:
            ...
            if sha256_crypt.verify(password_candidate, password):
                ...
                flash('A mail with a verification token has been sent to {}. Token is available for one minute'.format(
                    email), 'success')
...
def twoFactorAuthenticationMail(mailTo, username, subj='Hebrew Scripts Classifier Sign in token'):
    token = ''.join(secrets.choice(string.ascii_letters + string.digits) for i in
                    range(6))  # Generate 6 length token with letters and digits combined
    tokenSerialized = TOKEN_SERIALIZER.dumps(token)
    session[
        'token'] = tokenSerialized  # Save the serialized token on the session vars to compare it to the input from the user, within 60 seconds
...
def resetPassword():
    ...
    if request.method == 'POST' and form.validate():
        ...
        res = collection.find_one(
            {"username": username, "email": email})  # Get the details of the user by his mail and username
        ...
        flash('A mail with a reset password token has been sent to {}. Token is available for one minute'.format(email),
              'success')
...
def upload():
    if request.method == 'POST' and 'image' in request.files:
        ...
        os.replace(os.path.join(IMAGES_PATH, imageName),
                   os.path.join(IMAGES_PATH, folderID, imageName))  # Move the image to its folder
        results = runPredict(folderID=folderID)  # Run prediction on the folder, includes crop
        return render_template('upload.html', imageName=imageName, ashkenazi=results["ashkenazi"],
                               notAshkenazi=results["notAshkenazi"])
